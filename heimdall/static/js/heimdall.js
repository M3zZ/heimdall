...
(function($) {
    ...
    Node = (function() {
        function Node(row, tree, settings) {
            ...
            if (parentId != null && parentId !== "") this.parentId = parentId;
        ...
        Node.prototype.isBranchNode = function() {
            if (this.children.length > 0 || this.row.data(this.settings.branchAttr) === true) return true;
            else return false;
        };
        ...
        Node.prototype.parentNode = function() {
            if (this.parentId != null) return this.tree[this.parentId];
            else return null;
        };
        ...
        Node.prototype.render = function() {
            var handler, settings = this.settings,
                target;
            if (settings.expandable === true && this.isBranchNode()) {
                ...
                target.off("keydown.treetable").on("keydown.treetable", function(e) {
                    if (e.keyCode == 13) handler.apply(this, [e]);
                });
            }
        ...
        Node.prototype.reveal = function() {
            if (this.parentId != null) this.parentNode().reveal();
            return this.expand();
        };
        Node.prototype.setParent = function(node) {
            if (this.parentId != null) this.tree[this.parentId].removeChild(this);
        ...
        Node.prototype._initialize = function() {
            ...
            this.render();
            if (settings.expandable === true && settings.initialState === "collapsed") this.collapse();
            else this.expand();
            ...
            if (rows != null)
                for (i = 0; i < rows.length; i++) {
                    ...
                    if (row.data(this.settings.nodeIdAttr) != null) {
                        ...
                        if (node.parentId != null && this.tree[node.parentId]) this.tree[node.parentId].addChild(node);
    ...
    methods = {
        init: function(options, force) {
            ...
            return this.each(function() {
                ...
                if (force || el.data("treetable") === undefined) {
                    ...
                    if (settings.onInitialized != null) settings.onInitialized.apply(tree);
                }
                return el;
            });
        },
        ...
        , removeNode: function(id) {
            ...
            if (node) this.data("treetable").removeNode(node);
            else throw new Error("Unknown node '" + id + "'");
            return this;
        },
    };
    $.fn.treetable = function(method) {
        ...
        else return $.error("Method " + method + " does not exist on jQuery.treetable");
    };
...
heimdall.includeHTML = function(cb) {
  var z, i, elmnt, file, xhttp;
  z = document.getElementsByTagName("*");
  for (i = 0; i < z.length; i++) {
    elmnt = z[i];
    file = elmnt.getAttribute("heimdall-include-html");
    if (file) {
      xhttp = new XMLHttpRequest();
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          elmnt.innerHTML = this.responseText;
          elmnt.removeAttribute("heimdall-include-html");
          heimdall.includeHTML(cb);
        }
      }
...
function ruleSearchMatches(detail_leaf, keywords) {
    ...
    var match = true;
    var checked_keywords = detail_leaf.text().toLowerCase();
    var index;
...
function ruleSearch() {
    ...
    var matches = 0;
    $(".object-detail").each(function() {
        var detail_leaf = $(this);
        var detail_name = $(this).find(".object-name");
        if (ruleSearchMatches(detail_name, keywords)) {
            detail_leaf.removeClass("search-no-match");
            detail_leaf.addClass("search-match");
            ++matches;
        } else {
            detail_leaf.addClass("search-no-match");
            detail_leaf.removeClass("search-match");
       }
       detail_leaf.removeClass("last"); //clean this style from all objects
    });
    if (matches > 0){
        $(".search-match").each(function() {
            // search for the last object who matched
            if($(this)[0] === $(".search-match").last()[0]) {
                $(this).addClass("last");
            }
        });

    }
/*    if (!search_input) $("#search-matches").html("");
    else if (matches > 0) $("#search-matches").html(matches.toString() + " rules match.");
    else $("#search-matches").html("No rules match your search criteria!");*/
}

function deleteBySelection(item) {
    var object_to_delete = [];
    var object_name_to_delete = [];
    var id_to_delete = [];
    console.log(item)
    one_checked = false;
    $('.object-detail').each(function (){
        if ($(this).find('input')[0].checked){
            one_checked = true;
            object_to_delete.push($(this));
            //console.log($(this).find('.object-id a').text());
            if(item == 'audit'){
                object_name_to_delete.push('Audit #' + $(this).find('.object-id a').text());
            } else {
                object_name_to_delete.push($(this).find('.object-name a').text());
            }
            id_to_delete.push($(this).find('.object-id a').text());
        }
    });
    if (one_checked == true){
        if (confirm('Are you sure to want to delete : ' + object_name_to_delete.join(', ') )) {
               alert(object_name_to_delete.join(', ') + ' successfully deleted !');
               window.location.href = "http://localhost:8000/heimdall/item=" + item + "&action=remove&id=" + id_to_delete.join(',');
        }
    }
}
...
function updateItem(item, val) {
    window.location.href = "http://localhost:8000/heimdall/item=" + item + "&action=update&id=" + val.value;
}
